...
class UpdateProjectView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        project = get_object_or_404(Project, id = pk, submittedby = self.request.user)
        form = ProjectCreateForm(instance = project)
        return render(self.request, self.template_name, {'form': form})
    def post(self, request, pk):
        project = get_object_or_404(Project, id = pk, submittedby= self.request.user)
        form = ProjectCreateForm(request.POST, request.FILES or None, instance = project)
        if not form.is_valid():
            ctx = {'form': form}
            return render(self.request, self.template_name, ctx)
        project = form.save(commit = False)
...
def create_request(request, pk):
    if request.method == 'POST':
        ...
        created_request = Requests(request_project = project, person = self.request.user)
...
def delete_request(request, pk):
    if request.method == 'POST':
        ...
        requests = get_object_or_404(Requests, request_project = project, person = self.request.user)
        requests.delete()
        return redirect(reverse_lazy('index'))
    return redirect(reverse_lazy('index'))
